...
export async function POST(req: NextRequest) {
  try {
    // Prevent brute force attacks (max 10 login attempts per minute per IP)
    const limitResult = rateLimit(req, 10, 60 * 1000);
    ...
    // 1. Super Admin, Developer & Management Validation
    ...
    } else if (cleanEmail === 'developer@gauthamramkarthik.com' && cleanPassword === 'devpassword123') {
      authenticatedUser = {
        email: cleanEmail,
    ...
    } else if (cleanEmail === 'management@gauthamramkarthik.com' && cleanPassword === 'management123') {
      authenticatedUser = {
        email: cleanEmail,
        name: 'Total Website Management',
        role: 'Total Website Management',
        district: '',
      };
    } else {
