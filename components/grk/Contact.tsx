...
import { addWelfareRequest } from '@/lib/firebase/db';
...
export default function Contact({ onAdminClick }: { onAdminClick: () => void }) {
  const [form, setForm] = useState({
    ...
    email: '',
    phone: '',
    formType: 'enquiry', // 'enquiry' | 'welfare'
  ...
  const validate = () => {
    ...
    if (form.formType === 'enquiry') {
      if (!form.subject.trim()) e.subject = 'Subject is required';
    } else {
      if (!form.phone.trim()) e.phone = 'Phone number is required';
      else if (!/^\d{10}$/.test(form.phone.trim().replace(/\D/g, ''))) e.phone = 'Invalid phone number (must be 10 digits)';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      if (form.formType === 'welfare') {
        await addWelfareRequest({
          name: form.name,
          phone: form.phone,
            ...
            {sent ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                ...
                <div className="flex gap-4 p-1 bg-white/5 rounded-lg border border-white/5">
                  <button
                    type="button"
                    onClick={() => setForm(p => ({ ...p, formType: 'enquiry' }))}
                    className={`flex-1 py-2 text-xs font-black uppercase tracking-wider rounded-md transition-all duration-300 ${form.formType === 'enquiry' ? 'bg-[#FFD700] text-[#0B0F19] shadow-gold-glow' : 'bg-transparent text-gray-400 hover:text-white'}`}
                  >
                    General Enquiry
                  </button>
                  <button
                    type="button"
                    onClick={() => setForm(p => ({ ...p, formType: 'welfare' }))}
                    className={`flex-1 py-2 text-xs font-black uppercase tracking-wider rounded-md transition-all duration-300 ${form.formType === 'welfare' ? 'bg-[#FFD700] text-[#0B0F19] shadow-gold-glow' : 'bg-transparent text-gray-400 hover:text-white'}`}
                  >
                    Welfare Help Request
                  </button>
                </div>
                ...
                {form.formType === 'welfare' ? (
                  <>
                    <div>
                      <label className="block text-gray-300 text-[10px] font-black uppercase tracking-wider mb-2">Phone Number</label>
                      <div className="relative">
                        <span className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-gray-500"><Phone size={15} /></span>
                        <input type="text" name="phone" value={form.phone} onChange={handleChange} className={inputCls('phone')} placeholder="Enter 10-digit mobile number" />
                      </div>
                      {errors.phone && <p className="text-[#E50914] text-[10px] mt-1">{errors.phone}</p>}
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-gray-300 text-[10px] font-black uppercase tracking-wider mb-2">Welfare Category</label>
                        <select
                          name="category"
                          value={form.category}
                          onChange={handleChange}
                          className="w-full bg-[#161B26] border border-white/10 rounded-lg py-2.5 px-3 text-sm text-white focus:outline-none focus:border-[#FFD700]/50 transition-colors"
                        >
                      ...
                      <div>
                        <label className="block text-gray-300 text-[10px] font-black uppercase tracking-wider mb-2">Select District</label>
                        <select
                          name="district"
                          value={form.district}
                          onChange={handleChange}
                          className={`w-full bg-[#161B26] border rounded-lg py-2.5 px-3 text-sm text-white focus:outline-none focus:border-[#FFD700]/50 transition-colors ${errors.district ? 'border-[#E50914]' : 'border-white/10'}`}
                        >
                        ...
                        {errors.district && <p className="text-[#E50914] text-[10px] mt-1">{errors.district}</p>}
                      </div>
                    </div>
                  </>
                ) : (
          ...
          <div className="border-t border-white/6 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-5">
              <span className="text-xs text-gray-600 font-medium select-none">
                Developed by <span className="text-gray-500 font-semibold">Abishek.U</span>
              </span>
              <button
