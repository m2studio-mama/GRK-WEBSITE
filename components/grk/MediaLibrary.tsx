...
export default function MediaLibrary({ refreshKey }: Props) {
  ...
  const [lightboxTab, setLightboxTab] = useState<'photos' | 'wallpapers' | null>(null);
  ...
  const downloadSingleImage = (imgUrl: string, title: string, index: number) => {
    const a = document.createElement('a');
    a.href = imgUrl;
    const ext = imgUrl.split('.').pop()?.split(/[?#]/)[0] || 'jpg';
    a.download = `${title.replace(/\s+/g, '_')}_${index + 1}.${ext}`;
    a.target = '_blank';
  ...
  return (
    <section id="media-library" className="py-24 bg-[#0B0F19] relative overflow-hidden">
        ...
        {mainTab === 'photos' && (
          <div className="columns-1 sm:columns-2 md:columns-3 gap-5">
            {filteredPhotos.map((item, idx) => (
              <div
                key={item.id}
                onClick={() => { setLightboxIndex(idx); setLightboxTab('photos'); setSubImageIndex(0); setZoomScale(1); }}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filteredDownloads.map((asset, idx) => (
              <div key={asset.id} className="glass-card rounded-xl overflow-hidden border border-white/5 hover:border-[#FFD700]/20 transition-all duration-300 flex flex-col">
                <div onClick={() => { setLightboxIndex(idx); setLightboxTab('wallpapers'); setSubImageIndex(0); setZoomScale(1); }} className="aspect-[4/3] bg-black/60 relative overflow-hidden border-b border-white/5 cursor-pointer">
                  <img
      ...
      {/* Lightbox */}
      {lightboxIndex !== null && lightboxTab !== null && (
        (() => {
          const activeItems = lightboxTab === 'photos' ? filteredPhotos : filteredDownloads;
          const currentItem = activeItems[lightboxIndex];
          if (!currentItem) return null;
          const subImages = currentItem.url.split(',').map(u => u.trim()).filter(Boolean);
          const currentSubImageUrl = subImages[subImageIndex] || '';

          return (
            <div className="fixed inset-0 z-50 flex flex-col items-center justify-between p-4 bg-black/97 backdrop-blur-md" onClick={() => { setLightboxIndex(null); setLightboxTab(null); }}>
              {/* Header */}
              <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-3 py-2 px-4 z-10" onClick={e => e.stopPropagation()}>
                <div className="text-white text-xs font-semibold bg-black/40 px-3.5 py-2 rounded-full border border-white/5 flex items-center gap-2">
                  <span>{currentItem.title}</span>
                  <span className="text-[#FFD700] text-[9px] font-black uppercase tracking-widest bg-[#FFD700]/10 px-2 py-0.5 rounded-full border border-[#FFD700]/20">
                    {currentItem.category}
                  </span>
                </div>
                <div className="flex items-center gap-3 bg-black/60 px-4 py-1.5 rounded-full border border-white/10 flex-wrap justify-center">
                  ...
                  <button onClick={e => { e.stopPropagation(); downloadSingleImage(currentSubImageUrl, currentItem.title, subImageIndex); }} 
                    className="flex items-center gap-1 text-gray-400 hover:text-[#FFD700] text-[10px] font-bold uppercase transition-colors" aria-label="Download Current Image">
                    <Download size={14} /> <span className="hidden sm:inline">Image</span>
                  </button>
                  ...
                  {subImages.length > 1 && (
                    <>
                      <div className="w-px h-4 bg-white/10 mx-1" />
                      <button onClick={e => { e.stopPropagation(); triggerDownload(currentItem.url, currentItem.title); }} 
                        className="flex items-center gap-1 text-gray-400 hover:text-[#FFD700] text-[10px] font-bold uppercase transition-colors" aria-label="Download All ZIP">
                        <DownloadCloud size={14} /> <span className="hidden sm:inline">ZIP ({subImages.length})</span>
                      </button>
                    </>
                  )}

                  <div className="w-px h-4 bg-white/10 mx-1" />
                  <button onClick={() => { setLightboxIndex(null); setLightboxTab(null); }} className="text-gray-400 hover:text-[#FFD700] transition-colors" aria-label="Close"><X size={18} /></button>
                </div>
              </div>
              ...
              <div className="relative flex-grow w-full flex items-center justify-center overflow-hidden">
                {/* Prev Card */}
                <button onClick={e => { e.stopPropagation(); setLightboxIndex(i => (i !== null && i > 0 ? i - 1 : activeItems.length - 1)); setSubImageIndex(0); setZoomScale(1); }} 
                  className="absolute left-4 z-10 p-3 rounded-full bg-black/60 text-white hover:text-[#FFD700] border border-white/10" aria-label="Previous Package"><ChevronLeft size={20} /></button>
                
                {/* Center Image */}
                <div className="max-w-full max-h-[70vh] flex flex-col items-center justify-center transition-transform duration-200" style={{ transform: `scale(${zoomScale})` }}>
                  <img
                    src={currentSubImageUrl}
                    alt={currentItem.title}
                    className="max-w-[85vw] max-h-[60vh] object-contain rounded border border-white/10"
                    onClick={e => e.stopPropagation()}
                  />
                  
                  {/* Inside Package Slide Navigation */}
                  {subImages.length > 1 && (
                    <div className="mt-4 bg-black/80 px-3 py-1 rounded-full border border-white/10 flex items-center gap-3 z-10" onClick={e => e.stopPropagation()}>
                      <button onClick={() => setSubImageIndex(i => (i > 0 ? i - 1 : subImages.length - 1))} className="text-gray-400 hover:text-white transition-colors"><ChevronLeft size={14} /></button>
                      <span className="text-white text-[10px] font-black uppercase tracking-wider">Image {subImageIndex + 1} of {subImages.length}</span>
                      <button onClick={() => setSubImageIndex(i => (i < subImages.length - 1 ? i + 1 : 0))} className="text-gray-400 hover:text-white transition-colors"><ChevronRight size={14} /></button>
                    </div>
                  )}
                </div>

                {/* Next Card */}
                <button onClick={e => { e.stopPropagation(); setLightboxIndex(i => (i !== null && i < activeItems.length - 1 ? i + 1 : 0)); setSubImageIndex(0); setZoomScale(1); }} 
                  className="absolute right-4 z-10 p-3 rounded-full bg-black/60 text-white hover:text-[#FFD700] border border-white/10" aria-label="Next Package"><ChevronRight size={20} /></button>
              </div>

              {/* Package Sub-Image Thumbnails */}
              {subImages.length > 1 && (
                <div className="w-full flex justify-center gap-1.5 pb-2 overflow-x-auto" onClick={e => e.stopPropagation()}>
                  {subImages.map((subImg, idx) => (
                    <button key={idx} onClick={() => setSubImageIndex(idx)}
                      className={`flex-shrink-0 w-12 h-8 rounded overflow-hidden border transition-all ${subImageIndex === idx ? 'border-[#FFD700] scale-105 opacity-100' : 'border-white/10 opacity-50 hover:opacity-85'}`}
                    >
              ...
              {/* Package Library Cards Strip */}
              <div className="w-full flex justify-center gap-2 pb-2 overflow-x-auto border-t border-white/5 pt-3" onClick={e => e.stopPropagation()}>
                {activeItems.slice(0, 12).map((item, idx) => {
                  const firstImg = item.url.split(',')[0].trim();
                  return (
                    <button key={item.id} onClick={() => { setLightboxIndex(idx); setSubImageIndex(0); setZoomScale(1); }}
                      className={`flex-shrink-0 w-14 h-10 rounded overflow-hidden border-2 transition-all ${lightboxIndex === idx ? 'border-[#FFD700] opacity-100 scale-105' : 'border-white/10 opacity-50 hover:opacity-75'}`}
                    >
