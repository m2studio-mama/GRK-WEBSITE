...
export async function getRegistrations() {
  if (!pool) return [];
  try {
    await pool.query('ALTER TABLE registrations ADD COLUMN willingToDonate TINYINT(1) DEFAULT 0');
  } catch (err) {}
...
export async function addRegistration(reg: any) {
  ...
  await pool.query(
    'INSERT INTO registrations (id, name, phone, dob, district, city, email, photo, bloodGroup, status, joinedDate, willingToDonate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [regId, reg.name, reg.phone, reg.dob, reg.district, reg.city, reg.email, reg.photo || '', reg.bloodGroup, reg.status || 'Pending', joinedDate, reg.willingToDonate ? 1 : 0]
  );
  return { id: regId, ...reg, status: reg.status || 'Pending', joinedDate, willingToDonate: !!reg.willingToDonate };
}
...
export async function getDistricts() {
  if (!pool) return [];
  // Ensure presidentPhoto column exists in the MySQL table
  try {
    await pool.query('ALTER TABLE districts ADD COLUMN presidentPhoto LONGTEXT NULL');
  } catch (err) {
    // Silence error if the column already exists
  }
...
export async function addDistrict(item: any) {
  ...
  const id = 'd_' + Date.now();
  await pool.query(
    'INSERT INTO districts (id, districtName, groupLink, president, contactNumber, status, presidentPhoto) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [id, item.districtName, item.groupLink, item.president, item.contactNumber, item.status, item.presidentPhoto || '']
  );
  return { id, ...item };
}
...
// ─── WELFARE REQUESTS ─────────────────────────────────────────────
async function ensureWelfareRequestsTable() {
  if (!pool) return;
  await pool.query(`
...
export async function getWelfareRequests() {
  if (!pool) return [];
  await ensureWelfareRequestsTable();
  const [rows] = await pool.query('SELECT * FROM welfare_requests ORDER BY createdAt DESC');
  return rows;
}

export async function addWelfareRequest(item: any) {
  if (!pool) return null;
  await ensureWelfareRequestsTable();
  const id = 'wr_' + Date.now();
  await pool.query(
    'INSERT INTO welfare_requests (id, name, phone, district, category, message, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [id, item.name, item.phone, item.district, item.category, item.message, item.status || 'Pending']
  );
  return { id, ...item, status: item.status || 'Pending', createdAt: new Date().toISOString() };
}

export async function updateWelfareRequestStatus(id: string, status: string) {
  if (!pool) return null;
  await ensureWelfareRequestsTable();
  await pool.query('UPDATE welfare_requests SET status = ? WHERE id = ?', [status, id]);
  return { id, status };
}

export async function deleteWelfareRequest(id: string) {
  if (!pool) return;
  await ensureWelfareRequestsTable();
  await pool.query('DELETE FROM welfare_requests WHERE id = ?', [id]);
}
