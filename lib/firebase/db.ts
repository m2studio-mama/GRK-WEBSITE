...
const DB_VERSION = 'v13_fully_local';
...
const initialDistricts: District[] = TN_DIST_NAMES.map((name, idx) => ({
  id: `d${idx + 1}`,
  districtName: name,
  groupLink: `https://chat.whatsapp.com/grk-${name.toLowerCase()}`,
  president: `President of ${name}`,
  contactNumber: '+91 98765 43210',
  status: 'active',
  presidentPhoto: '/placeholder-user.jpg',
}));
...
function initLS() {
  ...
  if (localStorage.getItem('grk_db_version') !== DB_VERSION) {
    ['grk_movies','grk_gallery','grk_videos','grk_news','grk_coordinators',
     'grk_downloads','grk_fan_creations','grk_registrations','grk_timeline','grk_upcoming','grk_newsletter','grk_districts', 'grk_welfare_requests']
...
export const getWelfareRequests = async (): Promise<WelfareRequest[]> => {
  const mysqlData = await callApiProxy<WelfareRequest[]>('getWelfareRequests');
  if (mysqlData !== null) return mysqlData;

  if (mockMode) return lsGet<WelfareRequest>('grk_welfare_requests');
  return [];
};

export const addWelfareRequest = async (item: Omit<WelfareRequest, 'id' | 'status' | 'createdAt'>): Promise<WelfareRequest> => {
  const mysqlData = await callApiProxy<WelfareRequest>('addWelfareRequest', [item]);
  if (mysqlData !== null) return mysqlData;

  const newItem: WelfareRequest = {
    id: 'wr_' + Date.now(),
    ...item,
    status: 'Pending',
  ...
  if (mockMode) {
    const list = lsGet<WelfareRequest>('grk_welfare_requests');
    list.unshift(newItem);
    lsSet('grk_welfare_requests', list);
  }
  return newItem;
};

export const updateWelfareRequestStatus = async (id: string, status: 'Pending' | 'Resolved' | 'Contacted'): Promise<any> => {
  const mysqlData = await callApiProxy<any>('updateWelfareRequestStatus', [id, status]);
  if (mysqlData !== null) return mysqlData;
...
export const deleteWelfareRequest = async (id: string): Promise<void> => {
  await callApiProxy('deleteWelfareRequest', [id]);

  if (mockMode) {
    const list = lsGet<WelfareRequest>('grk_welfare_requests');
    const filtered = list.filter(r => r.id !== id);
    lsSet('grk_welfare_requests', filtered);
  }
};
